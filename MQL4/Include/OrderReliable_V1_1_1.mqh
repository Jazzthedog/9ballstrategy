...
// orders to the Metatrader server, dealing with transient connectivity problems
// better than the standard order sending functions.  It is essentially
// an error-checking wrapper around the existing transaction functions.
// This library provides nothing to help actual trade strategies, but ought to
// be valuable for nearly all expert advisors which trade 'live'. 
//
//						  Instructions:
//
...
// 
// If you see an an information warnings like these when you compile:
//
//       Function "OrderModifyReliableSymbol" is not referenced and will be removed from exp-file 
//
// that is OK, assuming you didn't actually intend to call the referred to function.  
//
//=============================================================================
//
//  Version:	1.1.1
...
/*                
   ...
   development.  In "regular" programming you can be pretty sure that operations
   will work as long as the computer is functioning.   This is not the
   case with transactions over the network, and you have to be quite careful
   and almost "paranoid" in the sense of checking for anything which might
   go wrong.  You must assume a possibility that all transactions might fail,
   and that much information may become unavailable, often transiently so.
   These failure modes will NOT be visible on a back-test, and are significantly
   more severe in a live "real-money" account than even real-time demo accounts.
   
   The problem is that many EA's use the existence, or nonexistence thereof
   of orders as a trigger for additional events, like setting up additional trades,
   in same direction or not.   If you assume that trades will be successful, or
   otherwise instantly visible to OrderSelect(), for example, you may be in for a
   shock as this information may be unavailable during a trade, or during the interval
   the dealing desk is 'holding' the order.  This may result in duplicate trades which
   shouldn't be there, in bad circumstances, a loop of a sequence of zillions of trades
   placed inappropriately.  If this is real money this could mean inappropriate risk to
   a catastrophe and the dealers will NOT bail you out as your bugs are your own problem. 
   
   Other problem is that Metatrader is somewhat amateurish in its design, and a real
   "pro"level financial system would have transaction-oriented multiple-phase commits
   and event queues.  On the downside this would be significantly more difficult
   to program.
   
   Unfortunately if you want to use Metatrader EA's there is no real alternative
   to learning how to write software fairly well.  A little bit of knowledge and
   over-confidence (and a buggy EA) is a very quick way to go bankrupt.   If you
   have never or almost never written software before, please go learn on something
   which will not cost you money.
   
   Specific suggestions:
   
*        Learn how to use the "MagicNumber" function in your EA's.  Remember
         that multiple EA's on the same symbol (perhaps even if different chart time frame's)
         will have ALL their orders visible to one another.  This is not the
         case in back-testing.   Many EA's which work fine in backtesting will totally
         get fried when there are multiple EA's working at once.
         Hence, if you want to see if an order is "owned"
         by an EA---which is ESSENTIAL!!!---you must check the MagicNumber variable
         in addition to the OrderSymbol() to ensure it is one of "yours". 
...
*        Manage your existing orders with the *ticket number*, i.e. the value
         ...
         by definition, the truly unique identifier of the order in the server's
         data base.  Use it.
         
*        Often you may combine the use of ticket numbers and magic numbers. 
         For instance, suppose you have potentially four different order kinds in
         your EA, two long orders (i.e. for scaling out), and two short orders.
         You would then define that each had a magic number *offset* from the
         magic number base.  E.g. you may have something like this:
...
void foo() {
    ...
    OrderSendReliable(  yadda yadda yadda,   my_MN, cstr,   yadda yadda ) 
         ...
         If you are in a loop checking existing orders with an OrderSelect() and a
         SELECT_BY_POS,  if you delete an order or close an order, then all other entries
         may become invalid!  By deleting then you have totally changed the list,
         and you ought to start over with a new loop once you have executed one
         client-server transaction.  For instance the OrdersTotal() can change.
...
*        If you have a condition that you think specifies a new order, then
         ...
         when it last occurred (and setting that to zero if the condition is false)
         and then only if it has remained true until (CurTime() - saved_time) >= some_interval
         of seconds. 
