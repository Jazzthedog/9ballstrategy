...
extern double BandsDeviations = 2.7;
...
int init()
{
   ...
   IndicatorBuffers(7);
         ...
         SetIndexBuffer(3,upArrow);   SetIndexStyle(3,DRAW_ARROW); SetIndexArrow(3,234);
         SetIndexBuffer(4,dnArrow);   SetIndexStyle(4,DRAW_ARROW); SetIndexArrow(4,233);
 	...
 	for(i = limit; i >= 0; i--)
   {
      ...
      //
         ...
         dnArrow[i] = EMPTY_VALUE;            
            if (High[i+1]>upBuffer[i+1] && Close[i+1]>Open[i+1] && Close[i]<Open[i]) upArrow[i] = High[i]+50*Point;
            if ( Low[i+1]<dnBuffer[i+1] && Close[i+1]<Open[i+1] && Close[i]>Open[i]) dnArrow[i] = Low[i]-150*Point;
