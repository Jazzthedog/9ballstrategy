...
/*
   ...
   ObjectSetString(ChartID(),"textlotsize",OBJPROP_TEXT,StringFormat("Volume:%.4f",lotSize));

*/
  color sArrow;
  int ticket1, ticket2, iSlipage,iSLPips,iTPPips;
  double SpreadInPoints;
  ...
  iSlipage=3;
  string sText="OrderSell_NNX2" + "_" + PeriodDesc(Period());
  sArrow=CLR_NONE;   //Order Arrow Color
  
  SpreadInPoints = (MarketInfo(Symbol(),MODE_SPREAD)*Point);
  ...
  ticket1=OrderSend(Symbol(), OP_SELL, lotSize, Ask, iSlipage, Ask+(ATRPoints*InpSLfactor), Ask-(ATRPoints*InpTPfactor)+SpreadInPoints, sText, 192, 0, sArrow);
  if(ticket1<0)
  {
     Alert("OrderSell_NNX ticket 1 returned the error of: ", GetErrorDescription(GetLastError()));
     Print("OrderSell_NNX ticket 1 returned the error of: ", GetErrorDescription(GetLastError()));
  }
  else
  {
   PlaySound ("ok.wav");
  ...
  ticket2=OrderSend(Symbol(), OP_SELL, lotSize, Ask, iSlipage, Ask+(ATRPoints*InpSLfactor), 0.0, sText, 192, 0, sArrow);
  if(ticket2<0)
  {
     Alert("OrderSell_NNX ticket 2 returned the error of: ", GetErrorDescription(GetLastError()));
     Print("OrderSell_NNX ticket 2 returned the error of: ", GetErrorDescription(GetLastError()));
  }
  else
  {
   PlaySound ("ok.wav");
   ...
   WindowScreenShot(FileName + "_SELL_" + Symbol() + "_" + IntegerToString(Period()) + "_" + newstr + ".gif", 800, 800, -1, -1, chartType);
   Print("Screenshot created at ", FileName + "_" + Symbol() + "_" + IntegerToString(Period()) + "_" + newstr + ".gif");
   PlaySound ("ok.wav"); 
   
  Print("Ask:", Ask, " (ATRPoints*InpSLfactor):", (ATRPoints*InpSLfactor), " (ATRPoints*InpTPfactor):", (ATRPoints*InpTPfactor), " Spread:", MarketInfo(Symbol(),MODE_SPREAD), " Spreadinpoints:", (MarketInfo(Symbol(),MODE_SPREAD)*Point) );  
}
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
