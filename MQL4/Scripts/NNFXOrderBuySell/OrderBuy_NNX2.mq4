...
/*
   ...
   ObjectSetString(ChartID(),"textspread",OBJPROP_TEXT,StringFormat("Spread:%s", DoubleToStr(MarketInfo(Symbol(),MODE_SPREAD),1)));
   

*/
  color sArrow;
  int ticket1, ticket2, iSlipage,iSLPips,iTPPips, SlippageInPoints;
  ...
  iSlipage=3;
  SlippageInPoints=30; // not sure why i am not using this points approach?
  
  string sText="OrderBuy_NNX2" + "_" + PeriodDesc(Period());
  ...
  ticket1=OrderSend(Symbol(), OP_BUY, lotSize, Ask, iSlipage, Ask-(ATRPoints*InpSLfactor), Ask+(ATRPoints*InpTPfactor)-SpreadInPoints, sText, 192, 0, sArrow);
  if(ticket1<0)
  {
  ...
  ticket2=OrderSend(Symbol(), OP_BUY, lotSize, Ask, iSlipage, Ask-(ATRPoints*InpSLfactor), 0.0, sText, 192, 0, sArrow);
  if(ticket2<0)
  {
     Alert("OrderBuy_NNX ticket 2 returned the error of: ", GetErrorDescription(GetLastError()));
     Print("OrderBuy_NNX ticket 2 returned the error of: ", GetErrorDescription(GetLastError()));
  }
  else
  {
   PlaySound ("ok.wav");
   ...
   WindowScreenShot(FileName + "_BUY2_" + Symbol() + "_" + IntegerToString(Period()) + "_" + newstr + ".gif", 800, 800, -1, -1, chartType);
