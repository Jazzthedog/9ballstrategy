...
int    Dgts=MarketInfo(Symbol(),MODE_DIGITS);     
   ...
   if(TakeProfit==true) TP=Bid-DistTP*Point;
   ticket=OrderSendReliableMKT(Symbol(),OP_SELL,LotsOptimized(),Bid,Ask-Bid,
                    NormalizeDouble(SL,Dgts),
                    NormalizeDouble(TP,Dgts),
                    "Hotkey",123456,0,CLR_NONE);
   if(ticket<=0) Alert("Error Open_SELL: ",ErrorDescription(OrderReliableLastErr())); 
   return(0);
}
//+------------------------------------------------------------------+
